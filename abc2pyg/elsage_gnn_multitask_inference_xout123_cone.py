...
from dataset_prep import PygNodePropPredDataset, Evaluator, EdgeListConeDataset
...
def main():
    ...
    elsage_model = GraphSAGE(in_dim=13,#dataset[0].num_node_features, #9 for gamora_output
                 hidden_dim=args.hidden_dim, 
                 out_dim=1, #predict only one bit
                 num_layers=args.num_layers,
                 dropout=args.dropout
                 ).to(device)
    ...
    for epoch in range(1, args.epochs + 1):
        loss, train_acc = train_ensemble(gamora_model, elsage_model, train_loader, optimizer, device, dataset, args.PO_bit)
        if epoch % 1 == 0:
            val_acc = test_ensemble_bit(gamora_model, elsage_model, val_loader, device, dataset, args.PO_bit)
            test_acc = test_ensemble_bit(gamora_model, elsage_model, test_loader, device, dataset, args.PO_bit)
