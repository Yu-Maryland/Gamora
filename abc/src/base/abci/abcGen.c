...
***********************************************************************/
void Abc_WriteMux( FILE * pFile )
{
    int fNaive = 0;
    fprintf( pFile, ".model MUX2\n" );
    fprintf( pFile, ".inputs a b s\n" ); 
    fprintf( pFile, ".outputs f\n" ); 
        fprintf( pFile, ".names a s x1\n" ); 
        fprintf( pFile, "11 1\n" ); 
        fprintf( pFile, ".names b s x2\n" ); 
        fprintf( pFile, "10 1\n" ); 
        fprintf( pFile, ".names x1 x2 f\n" ); 
        fprintf( pFile, "00 0\n" ); 
    fprintf( pFile, ".end\n" ); 
    fprintf( pFile, "\n" ); 
}

void Abc_GenMuxN( char * pFileName, int nVars ){
    FILE * pFile;
    pFile = fopen( pFileName, "w" );
    fprintf( pFile, "# %d-bit MUX generated by ABC on %s\n", nVars, Extra_TimeStamp() );
    int i, nDigits = Abc_Base10Log( nVars );
    assert( nVars >= 2 );
    assert( nVars % 2 == 0); // interested in 2^n to 1
    fprintf( pFile, ".model MUX%d\n", nVars );

    fprintf( pFile, ".inputs" );
    for ( i = 0; i < nVars; i++ )
        fprintf( pFile, " a%0*d", nDigits, i );
    for ( i = Abc_Base2Log(nVars)-1; i >= 0 ; i-- )
        fprintf( pFile, " s%0*d", nDigits, i );
 
    fprintf( pFile, "\n" );

    fprintf( pFile, ".outputs out\n" );

    fprintf( pFile, ".names c\n" );
    int prepin = 0;
    if ( nVars == 2 )
        fprintf( pFile, ".subckt MUX2 a=a0 b=b0 s=s0 f=z\n" );
    else
    {
        for ( int i2 = Abc_Base2Log(nVars); i2 > 1 ; i2-- )
		if(i2 == Abc_Base2Log(nVars)){
			for ( i = 0; i < pow(2,i2); i+=2 ){
            			fprintf( pFile, ".subckt MUX2 a=a%0*d b=a%0*d s=s%0*d f=%0*d\n", nDigits, i, nDigits, i+1, nDigits, i2-1, nDigits, i/2);
            			//fprintf( pFile, ".subckt MUX2 a=a%0*d b=b%0*d s=s%0*d f=%0*d\n", nDigits, i+1, nDigits, i+1, nDigits, i2, nDigits, i+1);
			}
			prepin += pow(2,i2)/2;
		}
		else{
			for ( i = 0; i < pow(2,i2); i+=2 ){
            			fprintf( pFile, ".subckt MUX2 a=%0*d b=%0*d s=s%0*d f=%0*d\n", nDigits, (int)(prepin - pow(2,i2)+i), nDigits, (int)(prepin - pow(2,i2)+1+i), nDigits, i2-1, nDigits, (int)(i/2 + prepin));
			}
			prepin += pow(2,i2)/2;
		}

        fprintf( pFile, ".subckt MUX2 a=%0*d b=%0*d s=s%0*d f=%s\n", nDigits, (int)(prepin-2), nDigits, (int)(prepin-1), nDigits, 0,  "out"); //final mux2
    }
    fprintf( pFile, ".end\n" ); 
    fprintf( pFile, "\n" );
    Abc_WriteMux(pFile);
