...
***********************************************************************/
int IoCommandWriteEqnTorch( Abc_Frame_t * pAbc, int argc, char **argv )
{
    char * pFileName;
    int c;
    ...
    pFileName = argv[globalUtilOptind];
    // call the corresponding file writer
    Io_Write( pAbc->pNtkCur, pFileName, IO_FILE_TORCH_AIG );
    return 0;
...
int IoCommandWriteTruth( Abc_Frame_t * pAbc, int argc, char **argv )
{
    ...
    //Extra_PrintHex( pFile, (unsigned *)pTruth, 1<<Abc_ObjFaninNum(pNode) ), fprintf( pFile, "\n" );
    fclose( pFile );
    Vec_IntFree( vTruth );
    return 0;
