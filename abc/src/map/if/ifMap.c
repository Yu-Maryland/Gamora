...
//        p->nBestCutSmall[1]++;
    ...
    assert(pObj->pCutSet);
    If_Cut_t * pObj_BestCut = If_ObjCutBest(pObj);
    assert(pObj_BestCut);
    int selfLevel = pObj->Level;
    printf("if-test: %d,%d,%d,%d,%d,%d,%d,%d,%d,%4.2f,%4.2f\n",  pObj->fCompl0, pObj->fCompl0,pObj->nRefs, pObj->pCutSet->nCutsMax, pObj->pCutSet->nCuts, 
            pObj_BestCut->nLeaves, pObj_BestCut->iCutFunc,
            pObj_BestCut->Cost, pObj_BestCut->Area,pObj_BestCut->Delay);
    If_Obj_t * pLeaf;
    If_CutForEachLeaf( p, pObj_BestCut, pLeaf, i ){
        printf("%d,",pLeaf->Level);
    }
    printf("\n");
...
/*cunxi
    If_ManForEachObj( p, pObj, i )
    {
        if(If_ObjIsCi(pObj) || If_ObjIsCo(pObj))
            continue;
        assert(pObj->nVisits);
        printf("if-test: %d\n", pObj->nVisits);
    }
