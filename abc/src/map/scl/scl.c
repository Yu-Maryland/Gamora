...
static int Scl_CommandMLSize     ( Abc_Frame_t * pAbc, int argc, char ** argv );
static int Scl_CommandMLDataset  ( Abc_Frame_t * pAbc, int argc, char ** argv );
...
//    Cmd_CommandAdd( pAbc, "SCL mapping",  "_buffer",       Scl_CommandBufferOld,   1 ); 
    ...
    Cmd_CommandAdd( pAbc, "SCL mapping",  "mlsize",        Scl_CommandMLSize,      1 ); 
    Cmd_CommandAdd( pAbc, "SCL mapping",  "mlgen",         Scl_CommandMLDataset,   1 ); 
...
***********************************************************************/
int Scl_CommandMLSize( Abc_Frame_t * pAbc, int argc, char **argv )
{
    ...
    pPars->fVeryVerbose  =    0;
    int fDirectLabel     =    0; 
    while ( ( c = Extra_UtilGetopt( argc, argv, "DLSG" ) ) != EOF )
    {
        switch ( c )
        {
        case 'D':
    ...
    if (fDirectLabel) 
    	Abc_ML_SclUpsizePerform2( (SC_Lib *)pAbc->pLibScl, pNtk, pPars , pFileName);
    else
    	Abc_ML_SclUpsizePerform( (SC_Lib *)pAbc->pLibScl, pNtk, pPars , pFileName);
    return 0;

usage:
    fprintf( pAbc->Err, "usage: mlsize [-DILG] id_class.csv\n" );
    fprintf( pAbc->Err, "\t           selectively increases gate sizes on the critical path\n" );
    fprintf( pAbc->Err, "\t-D         toggle directly applying label for sizing (default=False; applying to critical nodes)\n" );
    fprintf( pAbc->Err, "\t-h       : print the command usage\n");
    return 1;
}

int Scl_CommandMLDataset( Abc_Frame_t * pAbc, int argc, char **argv )
{
    SC_SizePars Pars, * pPars = &Pars;
    Abc_Ntk_t * pNtk = Abc_FrameReadNtk(pAbc);
    int c;
    ...
    while ( ( c = Extra_UtilGetopt( argc, argv, "LSG" ) ) != EOF )
    {
        switch ( c )
        {
        case 'L':
    ...
    Abc_ML_SclGenDataset((SC_Lib *)pAbc->pLibScl, pNtk, pPars);
    return 0;

usage:
    fprintf( pAbc->Err, "usage: mlsize [-ILG] id_class.csv\n" );
    fprintf( pAbc->Err, "\t           selectively increases gate sizes on the critical path\n" );
    fprintf( pAbc->Err, "\t-h       : print the command usage\n");
    return 1;
}
...
int Scl_CommandUpsize( Abc_Frame_t * pAbc, int argc, char **argv )
{
    ...
    pPars->fVerbose      =    0;
    pPars->fVeryVerbose  =    0;
    int fUseMLSizer          =    0; //MLSizer option
    Extra_UtilGetoptReset();
    while ( ( c = Extra_UtilGetopt( argc, argv, "IJWRNDGTXBLcsdvwh" ) ) != EOF )
    {
        switch ( c )
        {
        case 'I':
        ...
        case 'L':
            fUseMLSizer ^= 1;
            Abc_Print( -1, "Command line switch \"-L\" has been deleted.\n" );
            goto usage;
        case 'c':
            pPars->fUseWireLoads ^= 1;
            break;
