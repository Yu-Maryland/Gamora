/**CFile****************************************************************
  ...
  FileName    [sclMLSize.c]

  SystemName  [ABC: Logic synthesis and verification system.]

  PackageName [Standard-cell library representation.]

  Synopsis    [Selective increase of gate sizes using Graph Learning ]
...
std::string Abc_SclOnehotClass(int maxsize, int size)
{
    std::string onehot = "";
...
void Abc_ML_SclGenDataset(SC_Lib * pLib, Abc_Ntk_t * pNtk, SC_SizePars * pPars)
{
    SC_Man * p;
    p = Abc_SclManStart( pLib, pNtk, pPars->fUseWireLoads, pPars->fUseDept, 0, pPars->BuffTreeEst );
    std::string designName = pNtk->pName;
    const std::string extension = ".el";
    std::string name_with_extension = designName + extension;
    char *edgelist = &name_with_extension[0];
    Abc_SclEdgelistCunxi(p->pNtk, edgelist);
    printf("%sedgelist succesfully dumps into %s\n",KGRN,edgelist); 
    // cunxi: generate feature map 
    //cunxi feature maps
    const std::string extension_json = "-class_map.json";
    std::string classmap_with_extension = designName + extension_json; 
    std::string extension_csv = "-feats.csv";
    std::string feats_with_extension = designName + extension_csv;
 
    extension_csv = "-class_map.csv"; //format for DGL cktgl dataloader 
    std::string classmap_csv_with_extension;
    classmap_csv_with_extension = designName + extension_csv; // make space for the new std::string (should check the return value ...) 
    
    FILE * f_classmap, *f_feature, *f_classmap_csv;
    f_classmap = fopen (classmap_with_extension.c_str(), "w");
    f_feature = fopen (feats_with_extension.c_str(), "w");
    f_classmap_csv = fopen (classmap_csv_with_extension.c_str(), "w");
    fprintf(f_classmap, "{");
    Abc_Obj_t * pPiPo; int cunxi_i;
    //int maxSize = 1;
    int maxSize = Abc_SclMaxSizeCell(pLib);
    printf("Upsizing dataset generation\n -- max size = %d\n -- max #nodes = %d\n -- PI = %d\n -- PO = %d\n",
                        maxSize, Abc_NtkObjNumMax(pNtk), Abc_NtkPiNum(pNtk), Abc_NtkPiNum(pNtk));
    Abc_NtkForEachPi( p->pNtk, pPiPo, cunxi_i ){
	fprintf( f_classmap,"\"%d\":[%s], ", Abc_ObjId(pPiPo)-1, Abc_SclOnehotClass(maxSize,0).c_str()) ; //PI
	fprintf( f_classmap_csv,"%d,", 0) ; //PI
	fprintf( f_feature, "-1\n");
    }
    Abc_NtkForEachPo( p->pNtk, pPiPo, cunxi_i ){
	fprintf( f_classmap,"\"%d\":[%s], ", Abc_ObjId(pPiPo)-1, Abc_SclOnehotClass(maxSize,0).c_str()) ; //PO
	fprintf( f_classmap_csv,"%d,", 0) ; //PO
	fprintf( f_feature, "-2\n");
    }
    Abc_Obj_t * pCunxi_Obj; SC_Cell * pCellCunxi; int label=0;
    Abc_NtkForEachNode( p->pNtk, pCunxi_Obj, cunxi_i ){
        pCellCunxi = Abc_SclObjCell( pCunxi_Obj );
        label = pCellCunxi->Order;
        /*
        if(pCellCunxi->Order > 0)
		label = 1;
	else
		label = 0;
        */
	fprintf( f_classmap,"\"%d\":[%s]", Abc_ObjId(pCunxi_Obj)-1, Abc_SclOnehotClass(maxSize, label).c_str()) ; //PO
	fprintf( f_classmap_csv,"%d", label) ; 
...
/**Function*************************************************************
  ...
  Synopsis    [Assign Size to Object Nodes]
...
***********************************************************************/
void Abc_ML_SclAssignSizeToCell( SC_Man * p, Abc_Obj_t * pObj , int MLSize)
{
    ...
    {   
        //printf("working on the node %d\n", pObj->Id);
        //printf("-----Cell %s\n", pCellOld->pName);
	cell_iter ++; // cell size moved one size up.

	if (pCellNew->Order == MLSize){
...
***********************************************************************/
void Abc_ML_SclUpsizePerformInt( SC_Lib * pLib, Abc_Ntk_t * pNtk, SC_SizePars * pPars , std::map<int, int> pObjIDSize)
{
    SC_Man * p;
    ...
    //Abc_SclMinsizePerform(pLib, pNtk, 0, 0 ); // start with min-size network
    p = Abc_SclManStart( pLib, pNtk, pPars->fUseWireLoads, pPars->fUseDept, 0, pPars->BuffTreeEst );
    assert( p->vGatesBest == NULL );
    // perform upsizing -- method 1 working on critical path nodes
    /*
    for ( i = 0; i < 1; i++ )
    {
        for ( win = pPars->Window + ((i % 7) == 6); win <= 100;  win *= 2 )
        {
    	    ...
    	    Abc_ML_SclFindUpsizes( p, vPathNodes, pPars->Ratio, pPars->Notches, i, pPars->DelayGap, (pPars->BypassFreq > 0) );;
	}
    }
    */
    // perform upsizing -- method 2 working on all nodes in network 
    Abc_Obj_t * pObj;
    Abc_NtkForEachObj( p->pNtk, pObj, i ){
    	if(Abc_ObjIsPo(pObj) || Abc_ObjIsPi(pObj))
		continue;
	else{
		Abc_ML_SclAssignSizeToCell(p, pObj, pObjIDSize[pObj->Id]);	//assign random size 
	} 
    }
    Abc_NtkCleanMarkAB( pNtk );
    //update delay and area
...
void Abc_ML_SclManPrintGateSizes( SC_Lib * pLib, Abc_Ntk_t * p, Vec_Int_t * vGates )
{
    Abc_Obj_t * pObj;
    SC_Cell * pCell;
    ...
    Abc_NtkForEachNodeNotBarBuf1( p, pObj, i )
    {
        ...
        printf("Cell %s, Size %d\n", pCell->pName,pCell->Order);
...
void Abc_ML_SclPrintGateSizes( SC_Lib * pLib, Abc_Ntk_t * p )
{
    Abc_SclMioGates2SclGates( pLib, p );
    Abc_ML_SclManPrintGateSizes( pLib, p, p->vGates );
    Abc_SclSclGates2MioGates( pLib, p );
    Vec_IntFreeP( &p->vGates );
    p->pSCLib = NULL;
}

void Abc_ML_SclUpsizePerformInt2( SC_Lib * pLib, Abc_Ntk_t * pNtk, SC_SizePars * pPars , Vec_Int_t * vNodes)
{
    SC_Man * p;
    Vec_Int_t * vPathPos = NULL;    // critical POs
    Vec_Int_t * vPathNodes = NULL;  // critical nodes and PIs
...
Vec_Int_t * Abc_ML_SclGetMLNodes( Abc_Ntk_t * pNtk, char * pFileName)
{
    Vec_Int_t * vNodes = Vec_IntAlloc( 1000 ); 
    std::ifstream read_solution;
    read_solution.open(pFileName);
    std::string str;
    int threshold = Vec_PtrSize(pNtk->vPis) + Vec_PtrSize(pNtk->vPos);
    int id, c;
    //while(std::getline(read_solution,str)) {
    while(read_solution >> id >> c){
        if (id <= threshold)
		continue;
...
void Abc_ML_SclUpsizePerform( SC_Lib * pLib, Abc_Ntk_t * pNtk, SC_SizePars * pPars , char * pFileName)
{
    Abc_Ntk_t * pNtkNew = pNtk;
    if ( pNtk->nBarBufs2 > 0 )
        pNtkNew = Abc_NtkDupDfsNoBarBufs( pNtk );
    Vec_Int_t * vNodes = NULL; 
    vNodes = Abc_ML_SclGetMLNodes(pNtk, pFileName);
    Abc_Obj_t * pObj; int i;
    Abc_NtkForEachObjVec( vNodes, pNtk, pObj, i ){
	pObj->fMarkA = 1;
    }
    Abc_ML_SclUpsizePerformInt2( pLib, pNtkNew, pPars, vNodes);
    if ( pNtk->nBarBufs2 > 0 )
...
void Abc_ML_SclUpsizePerform2( SC_Lib * pLib, Abc_Ntk_t * pNtk, SC_SizePars * pPars , char * pFileName)
{
    Abc_Ntk_t * pNtkNew = pNtk;
    std::map <int,int> Obj_Size;
    std::map <int,int>::iterator Obj_Size_Iter;
    if ( pNtk->nBarBufs2 > 0 )
        pNtkNew = Abc_NtkDupDfsNoBarBufs( pNtk );
    Abc_Obj_t * pObj; int i;
    std::ifstream read_solution;
	...
	Obj_Size.insert(std::pair<int,int>(id, c)); // testing function : generating random size for all nodes
    	/*
	if (c>5)
		Obj_Size.insert(std::pair<int,int>(id, 5)); // testing function : generating random size for all nodes
	else
		Obj_Size.insert(std::pair<int,int>(id, c)); // testing function : generating random size for all nodes
	*/
		
    }
    printf("%d, %d\n", pNtk->nObjs, Obj_Size.size());
    assert(pNtk->nObjs == Obj_Size.size()); 
    //for (Obj_Size_Iter = Obj_Size.begin(); Obj_Size_Iter != Obj_Size.end(); Obj_Size_Iter++)
    //    std::cout << Obj_Size_Iter->first<< " " <<Obj_Size_Iter->second<<std::endl;
    Abc_ML_SclUpsizePerformInt( pLib, pNtkNew, pPars , Obj_Size);
    if ( pNtk->nBarBufs2 > 0 )
        Abc_SclTransferGates( pNtk, pNtkNew );
    if ( pNtk->nBarBufs2 > 0 )
        Abc_NtkDelete( pNtkNew );
    printf("MLSize finished\n");
}
