...
void Mio_WriteGateTorch( FILE * pFile, Mio_Gate_t * pGate, int GateLen, int NameLen, int FormLen, int fPrintSops, int fAllPins )
{
    char Buffer[5000];
    assert( NameLen+FormLen+2 < 5000 );
    sprintf( Buffer, "%s",    pGate->pForm );
    //sprintf( Buffer, "%s=%s;",    pGate->pOutName, pGate->pForm );
    Mio_Pin_t * pPin;
    fprintf( pFile, "def %s(", pGate->pName );
    Mio_GateForEachPin( pGate, pPin )
    	fprintf( pFile, "%s,", pPin->pName );
   fprintf( pFile, "):\n");

   // fprintf( pFile, "%s\n",       pGate->pSop? pGate->pSop : "unspecified\n" );
//    fprintf( pFile, "%8.2f  ",    pGate->dArea );
//    fprintf( pFile, "%-*s ",      Abc_MinInt(NameLen+FormLen+2, 60), Buffer );
    // print the pins
/*
...
void Mio_WriteLibraryTorch( FILE * pFile, Mio_Library_t * pLib, int fPrintSops, int fShort, int fSelected )
{
    Mio_Gate_t * pGate;
    Mio_Pin_t * pPin;
    Vec_Ptr_t * vGates = Vec_PtrAlloc( 1000 );
    ...
    fprintf( pFile, "# The Python-Torch Genlib library \"%s\" with %d gates written by ABC on %s\n", pLib->pName, Vec_PtrSize(vGates), Extra_TimeStamp() );
    Vec_PtrForEachEntry( Mio_Gate_t *, vGates, pGate, i ){
   	//if(Mio_GateReadPinNum(pGate)>2)
	//	continue;
	Mio_WriteGateTorch( pFile, pGate, GateLen, NameLen, FormLen, fPrintSops, fAllPins );
    }
    Vec_PtrFree( vGates );
}
