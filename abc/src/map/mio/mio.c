...
int Mio_CommandWriteGenlib( Abc_Frame_t * pAbc, int argc, char **argv )
{
    ...
    Extra_UtilGetoptReset();
    while ( (c = Extra_UtilGetopt(argc, argv, "vath")) != EOF ) 
    {
        switch (c) 
        {
            case 'v':
    ...
    if (!fTorch)
    	...
    	Mio_WriteLibraryTorch( pFile, (Mio_Library_t *)Abc_FrameReadLibGen(), 0, 0, fSelected );
	
    fclose( pFile );
    if (!fTorch)
    	printf( "The current genlib library is written into file \"%s\".\n", pFileName );
    else
    	printf( "The current Python-Torch genlib library is written into file \"%s\".\n", pFileName );
...
usage:
    ...
    fprintf( pErr, "\t-t      : toggles writing torch library for torch logical [default = %s]\n", fTorch? "yes" : "no" );
    fprintf( pErr, "\t-h      : print the command usage\n");
    fprintf( pErr, "\t<file>  : optional file name to write the library\n");
    return 1;       
}

int Mio_CommandWriteGenlibTorch( Abc_Frame_t * pAbc, int argc, char **argv )
{
    FILE * pOut, * pErr, * pFile;
    char * pFileName;
    int fSelected = 0;
